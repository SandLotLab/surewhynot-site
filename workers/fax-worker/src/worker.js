...
export default {
  async fetch(req, env, ctx) {
    ...
    // Temporary diagnostics route (no secrets exposed) to verify the active Stripe key format.
    if (url.pathname === "/api/_debug/stripe" && req.method === "GET") {
      const info = getStripeKeyInfo(env);
      return json(
        {
          configured: info.configured,
          source: info.source,
          prefix: info.prefix,
          startsWithSk: info.startsWithSk,
          length: info.length,
        },
        200
      );
...
function getStripeKeyInfo(env) {
  const keyCandidates = [
    ["STRIPE_SECRET_KEY", env.STRIPE_SECRET_KEY],
    ["STRIPE_API_KEY", env.STRIPE_API_KEY],
  ];

  for (const [source, raw] of keyCandidates) {
...
function getStripeSecretKey(env) {
  const info = getStripeKeyInfo(env);
  if (!info.configured || !info.startsWithSk) return null;
  return info.key;
}
...
async function createCheckout(req, env) {
  ...
  const keyInfo = getStripeKeyInfo(env);
  const stripeSecretKey = getStripeSecretKey(env);
  if (!stripeSecretKey) {
    return json(
      {
        error: "Stripe is not configured. Set STRIPE_SECRET_KEY (or STRIPE_API_KEY) to a valid secret key that starts with sk_.",
        stripeKeyInfo: {
  ...
  return json({ url: j.url, id: j.id, pricing, stripeKeySource: keyInfo.source }, 200);
}
